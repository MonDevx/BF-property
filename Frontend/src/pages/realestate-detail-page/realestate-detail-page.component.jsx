...
function PropertyDetailpage(props) {
  ...
  useEffect(() => {
    axios
      .get(
        `https://us-central1-bfproperty.cloudfunctions.net/webApi/api/v1/realestatedetail/${props.match.params.name}`
      )
      .then((result1) => {
        const propertyData = result1.data.data;
        setProperty(propertyData);
        setId(result1.data.id);
        axios({
          method: "GET",
          ...
          .then((result2) => {
            setNearbyplacesapi(result2.data.data);
            setIsLoading(false);
          })
          .catch((error) => {
            alert.error(error);
